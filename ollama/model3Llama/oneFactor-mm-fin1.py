...
# Load data
data = load_data("dataV3llama.csv", N_STEPS, scale=SCALE, shuffle=SHUFFLE, test_size=TEST_SIZE)
...
def get_final_df(model, data):
    ...
    if SCALE:
        y_test = np.squeeze(data["column_scaler"]["Score"].inverse_transform(np.expand_dims(y_test, axis=0)))
        y_pred = np.squeeze(data["column_scaler"]["Score"].inverse_transform(y_pred))
...
def predict(model, data):
    ...
    if SCALE:
        predicted_change = data["column_scaler"]["Score"].inverse_transform(prediction)[0][0]
    else:
